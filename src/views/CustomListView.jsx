...
const CustomListView = ({ onShowDetails, customList, onAddMovie, onRemoveMovie, error, setError }) => {
  const [selectedMovie, setSelectedMovie] = useState(null);
  const [isSpinning, setIsSpinning] = useState(false);
  const [watchProviders, setWatchProviders] = useState(null);

  const fetchWatchProviders = async (movieId, mediaType) => {
    try {
  ...
  const handleSpinCustomList = () => {
    ...
    if (customList.length < 2) {
      setError("Adicione pelo menos 2 itens para sortear.");
      return;
    }
    setIsSpinning(true);
    ...
    setTimeout(async () => {
      ...
      const mediaType = chosenMovie.title ? 'movie' : 'tv';
      
      await fetchWatchProviders(chosenMovie.id, mediaType);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto flex flex-col lg:flex-row gap-8 lg:gap-12">
      <div className="lg:w-2/5 lg:max-w-md flex-shrink-0 space-y-6">
        <div>
          <h2 className="text-2xl font-bold text-white mb-4">Monte sua Lista (até 10 itens)</h2>
        ...
        <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-700/50 flex flex-col">
          <h3 className="font-bold mb-3 text-slate-300 flex-shrink-0">Itens na Lista: {customList.length}/10</h3>
          {customList.length === 0 ? (
            <div className="flex flex-col items-center justify-center text-center text-slate-500 h-full py-10 flex-grow">
              ...
              <p>Sua lista está vazia. Use a busca acima.</p>
            </div>
          ) : (
            <ul className="space-y-2 max-h-96 overflow-y-auto pr-2">
              {customList.map((movie, index) => (
                <li key={movie.id} className="flex items-center justify-between p-2 bg-slate-700/50 rounded-md animate-fade-in">
                  <div className="flex items-center gap-3 min-w-0">
                    <span className="font-bold text-cyan-400 w-6 text-center flex-shrink-0">{index + 1}.</span>
                    <img src={`https://image.tmdb.org/t/p/w92${movie.poster_path}`} alt={movie.title || movie.name} className="w-10 h-14 rounded-sm object-cover flex-shrink-0" />
                    <p className="text-sm truncate flex-grow min-w-0">{movie.title || movie.name}</p>
                  </div>
                  <button onClick={() => onRemoveMovie(movie.id)} className="text-slate-500 hover:text-red-500 ml-2 flex-shrink-0">
