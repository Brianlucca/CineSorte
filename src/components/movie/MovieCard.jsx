...
import { InformationCircleIcon, StarIcon, BookmarkIcon } from '@heroicons/react/24/solid';
...
const MovieCard = ({ item, allGenres, onShowDetails, watchProviders }) => {
  ...
  const { isModalOpen, openModal, closeModal, userLists, isLoading, addMovieToSelectedList, movieToAdd } = useAddToList();
  ...
  const getGenreNames = (genreIds) => {
    if (!genreIds || !Array.isArray(allGenres) || allGenres.length === 0) return [];
    ...
    <>
      ...
      <div className="group relative w-full max-w-4xl rounded-2xl overflow-hidden shadow-2xl shadow-black/60 border border-slate-800 bg-slate-900/70 backdrop-blur-sm animate-fade-in transition-all duration-300 hover:border-cyan-500/50">
        <div className="absolute inset-0 z-0">
          {backdropUrl && (
            <img src={backdropUrl} alt="" className="w-full h-full object-cover blur-xl scale-125 opacity-20 transition-opacity duration-300" />
          )}
          <div className="absolute inset-0 bg-gradient-to-r from-slate-900 via-slate-900/80 to-transparent"></div>
          <div className="absolute top-0 right-0 bottom-0 left-1/2 bg-gradient-to-b from-cyan-500/20 to-transparent rounded-full opacity-0 group-hover:opacity-50 transition-opacity duration-500 blur-3xl -translate-y-1/2 translate-x-1/4" aria-hidden="true"></div>
        </div>
          ...
          <div className="flex flex-col text-center md:text-left flex-grow">
            <div>
              <div className="flex justify-between items-start">
                  ...
                  {currentUser && (
                    <button onClick={() => openModal(item)} className="p-2 rounded-full bg-slate-800/60 text-slate-400 hover:text-cyan-400 hover:bg-slate-700 transition-colors flex-shrink-0">
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mt-4 mb-4">
                ...
                {itemGenres.slice(0, 2).map(genre => (
                  <span key={genre} className="bg-slate-700/50 text-slate-300 text-xs font-semibold px-2.5 py-1 rounded-full hidden sm:inline-block">{genre}</span>
                ))}
              </div>
            ...
            <div className="mt-auto flex flex-col gap-6 pt-6">
              {watchProviders && (
                <div>
                  <h3 className="font-bold text-slate-400 mb-2 text-sm text-left">Disponível em:</h3>
                  <div className="flex flex-wrap gap-3 justify-center md:justify-start">
                    {watchProviders.length > 0 ? (
                      watchProviders.slice(0, 5).map(provider => (
                        <img key={provider.provider_id} src={`https://image.tmdb.org/t/p/w92${provider.logo_path}`} alt={provider.provider_name} className="w-10 h-10 rounded-lg" title={provider.provider_name} />
                      ))
                    ) : (
                      <p className="text-xs text-slate-500 text-left">Nenhum serviço de streaming (assinatura) encontrado.</p>
                    )}
                  </div>
                </div>
              )}
              ...
              <div className="border-t border-slate-700/50 pt-6 flex justify-center md:justify-start">
                <button onClick={() => onShowDetails(item.id, mediaType)} className="inline-flex items-center gap-2 bg-slate-800 hover:bg-cyan-500 border border-slate-700 hover:border-cyan-500 transition-all duration-300 text-white font-bold py-2 px-5 rounded-lg shadow-lg shadow-black/50">
